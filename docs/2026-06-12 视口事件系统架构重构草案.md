...
12. 业务 Controller 只处理 `ViewportInteractionEvent`，不直接处理原始 DOM 事件或 `ViewportRawInputEvent`。
13. `pointer move` 只表示无按键移动；按键按下后的移动必须解释成 drag 系列事件。
...
原始 DOM 事件和 `ViewportRawInputEvent` 只允许 `ViewportInteractor` 使用，具体 Command、Camera、Selection、ViewCube 不直接处理原始 pointer 事件。
...
- 可以继承 `DefaultEventController`。
- 如果框选等选择操作需要连续状态，应由 `SelectionController` 自己维护。

...
10. 相机等基础交互控制器也可以继承 `DefaultEventController`。
...
- 当前 `EditorViewportRuntime` 和 `ViewportInteractionController` 中混在一起的分发逻辑。
...
classDiagram
    ...
    DefaultEventController <|-- EditorDefaultController
    Editor <|-- ModelEditor
    Editor <|-- SketchEditor
    ...
    EventHandler --> ViewportMouseEvent : handles
    EventHandler --> ViewportKeyboardEvent : handles
    CommandManager o-- Command : delegates to active command
    Editor o-- EventHandler : default handlers
...
它是给 `EventHandler` 和具体 Controller 使用的事件，不是浏览器原生事件的直接暴露。
...
所有能参与事件分发的对象都实现这个接口。它接收的是 `ViewportInteractor` 生成的 `ViewportMouseEvent` 或 `ViewportKeyboardEvent`，不直接接收浏览器原生事件。

```ts
interface EventHandler {
    onPointerDown(event: ViewportMouseEvent): boolean;
    onPointerMove(event: ViewportMouseEvent): boolean;
    onPointerUp(event: ViewportMouseEvent): boolean;
    onClick(event: ViewportMouseEvent): boolean;
    onDoubleClick(event: ViewportMouseEvent): boolean;
    onLeftDragStart(event: ViewportMouseEvent): boolean;
    onLeftDrag(event: ViewportMouseEvent): boolean;
    onLeftDragStop(event: ViewportMouseEvent): boolean;
    onLeftDragCancel(event: ViewportMouseEvent): boolean;
    onMiddleDragStart(event: ViewportMouseEvent): boolean;
    onMiddleDrag(event: ViewportMouseEvent): boolean;
    onMiddleDragStop(event: ViewportMouseEvent): boolean;
    onMiddleDragCancel(event: ViewportMouseEvent): boolean;
    onRightDragStart(event: ViewportMouseEvent): boolean;
    onRightDrag(event: ViewportMouseEvent): boolean;
    onRightDragStop(event: ViewportMouseEvent): boolean;
    onRightDragCancel(event: ViewportMouseEvent): boolean;
    onWheel(event: ViewportMouseEvent): boolean;
    onKeyDown(event: ViewportKeyboardEvent): boolean;
}
```
...
class CommandManager implements EventHandler {
    onPointerDown(event: ViewportMouseEvent): boolean {
        ...
        return this.activeCommand.onPointerDown(event);
    }
}
```
...
- 通过 `onLeftPointerDown`、`onLeftClick`、`onLeftDrag`、`onPointerMove`、`onWheel`、`onKeyDown` 等回调处理和自身业务相关的输入。
- 具体命令只重写自己关心的事件。
...
这些默认 Controller 可以继承 `DefaultEventController`，也可以只实现 `EventHandler`。
...
也就是它负责“听到事件、整理事件、发给别人处理”。它不关心后面具体是 `CommandManager`、`Editor` 还是相机，只认 `EventHandler` 接口。
...
- `CameraNavigationController` 这类基础交互控制器也可以继承 `DefaultEventController`。

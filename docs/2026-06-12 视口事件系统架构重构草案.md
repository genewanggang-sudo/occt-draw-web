...
这版设计先确认核心类名、依赖关系和事件语义规则，不展开完整环境切换、事务和包内落点方案。
...
13. 业务 Controller 只处理 `ViewportInteractionEvent`，不直接处理原始 DOM 事件或 `ViewportRawInputEvent`。
...
classDiagram
    ...
    class DrawingEditor
    class ModelDefaultController
    class SketchDefaultController
    class DrawingDefaultController
    ...
    DefaultEventController <|-- ModelDefaultController
    DefaultEventController <|-- SketchDefaultController
    DefaultEventController <|-- DrawingDefaultController
    Editor <|-- ModelEditor
    Editor <|-- SketchEditor
    ...
    Editor --> HighlightState : updates
    RenderEngine --> SelectionState : renders selected
    RenderEngine --> HighlightState : renders highlighted
```
...
它是给 `EventHandler` 和具体 Controller 使用的事件，不是原生事件的简单包装，而是经过 `ViewportInteractor` 根据 pointer 状态加工后的语义事件。
...
`ViewportInteractionEvent` 是业务 Controller 唯一应该处理的视口输入事件。
...
例如草图画线命令可以使用 `onPointerMove` 更新预览线、snap 点、动态输入和光标坐标；也可以使用 `onLeftDrag` 支持按住左键拖拽画线。
...
- `HighlightState` 记录当前预选、高亮或 hover 命中了什么。
- 当前 `Editor` 环境下的默认交互负责根据输入事件更新这些状态。
- 渲染层根据 `SelectionState` 和 `HighlightState` 决定最终显示效果。

这样可以避免把事件解释、选择数据和渲染表现混在一个 `SelectionController` 里。

不同环境可以有不同的选择规则。例如建模环境可以选择实体、面、边；草图环境可以选择草图点、线、圆弧、约束；工程图环境可以选择视图、标注、图纸元素。这些规则应属于当前 `Editor` 或其默认 Controller，而不是全局固定的 `SelectionController`。
...
- 当前 `Editor` 的默认 Controller 可以记录框选、预选等环境级交互状态。

事件是否被消费，只由分发链根据 `EventHandler.handleEvent` 的返回值决定。
...
其中选择和高亮的默认交互由当前 `Editor` 或其默认 Controller 处理，不作为单独的顶层 `SelectionController` 放入分发链。
...
- `SelectionState`、`HighlightState` 是状态或服务，不作为顶层事件 Controller。
- 选择、高亮和环境默认交互由当前 `Editor` 或其默认 Controller 处理。
- `ModelEditor`、`SketchEditor`、`AssemblyEditor`、`DrawingEditor` 当前只确认设计原则，完整职责和包内落点后续单独设计。
- 当前默认处理顺序为：`ViewCubeController`、`CommandManager`、`Editor`、`CameraNavigationController`。

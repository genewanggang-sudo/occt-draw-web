...
flowchart TD
    ...
    Tx --> Apply["apply once to live document"]
    Apply --> History["History / Undo / Redo"]
```
...
- 自动打开 MutationScope。
- 自动生成 Transaction。
- 只通过 Transaction 更新 live document。
- 自动写入 History。
...
4. legacy `Request` 继续按旧链路执行。
5. `DocumentEditor.execute()` 同时支持 legacy Request。
6. 新 DocumentRequest 只在 MutationScope 能生成 transaction 后再开放业务迁移。
7. 为 core 增加测试，覆盖 legacy execute / undo / redo 基础流程。
...
4. `DocumentEditor` 只通过 transaction apply live document 一次。
5. request 失败时 discard scope，live document 不变。
6. 空 transaction 不进入 history。

验收：

- 不存在 request 修改 live 后又 apply transaction 的路径。
- 异常时 live document 不变。
- transaction apply 只发生一次。
- undo/redo 行为正确。
